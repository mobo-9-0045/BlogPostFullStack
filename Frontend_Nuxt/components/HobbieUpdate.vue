<template>
    <div class="grid grid-cols-1 h-full items-center justify-center w-full">
      <div class="w-full p-8 rounded h-auto">
        <h1 class="text-2xl font-bold text-center mb-6">Edit Hobbies</h1>
        ...
        <div v-if="showAddSkillModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div class="bg-white p-6 rounded shadow-lg max-w-lg w-full">
            <h2 class="text-xl font-bold mb-4">Add New Hobbie</h2>
  
            <!-- New Project Name -->
            <div class="mb-4">
              <label for="newProjectName" class="block text-sm font-medium text-gray-700">Hobbie Name:</label>
              <input
                type="text"
                v-model="newHobbie.hobbieName"
            ...
            <div class="mb-4">
              <label for="newProjectDescription" class="block text-sm font-medium text-gray-700">Hobbie Description:</label>
              <textarea
                v-model="newHobbie.hobbieDescription"
                class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                required
              ></textarea>
            </div>
        ...
        >
          ...
          <div class="mb-4">
            ...
            <textarea
              v-model="hobbie.hobbieDescription"
              class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
              required
            ></textarea>
          </div>
  ...
  import { ref } from 'vue';
  import { useUserStore, type Hobbie } from '../stores/user';
  ...
  const hobbies = ref(UserStore.hobbies);
  ...
  const updateHobbie = async (hobbie: Hobbie) => {
    const token = localStorage.getItem('token');
    try {
      const res = await axios.put(
        `http://127.0.0.1:3000/hobbie/updateHobbie/${hobbie.id}`,
        {
            hobbieName: hobbie.hobbieName,
            hobbieDescription: hobbie.hobbieDescription,
        },
        {
          headers: {
  ...
  const deleteHobbie = async (id: number) => {
    const token = localStorage.getItem('token');
    try {
      const res = await axios.delete(
        `http://127.0.0.1:3000/hobbie/deleteHobbie/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const addHobbie = () =>{
    const token = localStorage.getItem('token');
    try{
      const res = axios.post('http://127.0.0.1:3000/hobbie/createHobbie',{
        hobbieName: newHobbie.value.hobbieName,
        hobbieDescription: newHobbie.value.hobbieDescription,
      },
      {
        headers:{
          Authorization: `Bearer ${token}`,
