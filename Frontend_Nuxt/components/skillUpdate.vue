<template>
    <div class="grid grid-cols-1 h-full items-center justify-center w-full">
      <div class="w-full p-8 rounded h-auto">
        <h1 class="text-2xl font-bold text-center mb-6">Edit Skills</h1>
        ...
        <div v-if="showAddSkillModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div class="bg-white p-6 rounded shadow-lg max-w-lg w-full">
            ...
            <div class="mb-4">
              <label for="newProjectDescription" class="block text-sm font-medium text-gray-700">Project Description:</label>
              <textarea
                v-model="newSkill.skillDescription"
                class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                required
              ></textarea>
            </div>
  ...
  const skills = ref(UserStore.skills);
  ...
  const updateSkill = async (skill: Skill) => {
    ...
    try {
      const res = await axios.put(
        `http://127.0.0.1:3000/skills/updateSkill/${skill.id}`,
        {
  ...
  const deleteSkill = async (id: number) => {
    ...
    try {
      const res = await axios.delete(
        `http://127.0.0.1:3000/skills/deleteSkill/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const addSkill = () =>{
    const token = localStorage.getItem('token');
    try{
      const res = axios.post('http://127.0.0.1:3000/skills/createSkill',{
        skillName: newSkill.value.skillName,
        skillDescription: newSkill.value.skillDescription,
      },
      {
        headers:{
          Authorization: `Bearer ${token}`,
